...
class PredictionResults:
    ...
    def calculate_gains(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Close' not in df.columns or 'Open' not in df.columns:
            raise ValueError("Necessary columns missing in the dataframe.")
        df['PercentGain'] = (df['Close'] - df['Open']) / df['Open'] * 100
    ...
    def init_instance(cls) -> None:
        ...
        cls.filtered_df['Symbol'].to_csv('predictioned_symbols.csv', index=False)
    ...
    def compute_gains(cls) -> None:
        ...
        for symbol in cls.filtered_df['Symbol'].unique():
            # Create StockData object and get data for a particular stock symbol
            stock_data = StockData('predictioned_symbols.csv', start_time, end_time)
            symbol_df = stock_data._get_stock_data(symbol)  # assuming this method returns a dataframe
